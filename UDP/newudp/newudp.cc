...
static class newUdpAgentClass : public TclClass {
public:
	newUdpAgentClass() : TclClass("Agent/newudp") {}
	TclObject* create(int, const char*const*) {
		return (new newUdpAgent());
	}
} class_udp_agent;

newUdpAgent::newUdpAgent() : Agent(PT_NEWUDP), seqno_(-1)
{
	bind("packetSize_", &size_);
}

newUdpAgent::newUdpAgent(packet_t type) : Agent(type)
{
	bind("packetSize_", &size_);
}
...
// have one.
void newUdpAgent::sendmsg(int nbytes, AppData* data, const char* flags)
{
	Packet *p;
	int n;
...
void newUdpAgent::recv(Packet* pkt, Handler*)
{
	if (app_ ) {
...
int newUdpAgent::command(int argc, const char*const* argv)
{
	if (argc == 4) {
